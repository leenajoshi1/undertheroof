...
export function createScriptedMunicipalTurn(spatial: boolean, injectBadCitation = false): ModelTurn {
  ...
  if (spatial) plan.push(['record_spatial_graph', { coordinateSystem: 'normalized_0_to_1', geometryStatus: 'approximate', rooms: [
    { id: 'kitchen', label: 'Kitchen (fallback graph fixture)', bounds: { x: 0.08, z: 0.16, width: 0.39, depth: 0.59 }, adjacentTo: ['dining'], evidenceIds: ['demo:floor-plan', 'demo:photo'], confidence: 'medium' },
    { id: 'dining', label: 'Dining (fallback graph fixture)', bounds: { x: 0.5, z: 0.16, width: 0.42, depth: 0.59 }, adjacentTo: ['kitchen'], evidenceIds: ['demo:floor-plan'], confidence: 'medium' },
  ], connections: [{ from: 'kitchen', to: 'dining', type: 'open_transition', evidenceIds: ['demo:floor-plan'] }], observations: [{ id: 'kitchen-layout', area: 'kitchen', observation: 'The supplied synthetic floor plan and kitchen reference image suggest the kitchen configuration may warrant documentation review.', evidenceIds: ['demo:floor-plan', 'demo:photo', 'demo:claim'], confidence: 'medium', investigationSuggestion: 'Check permit records for renovation or plumbing work.' }] }]);
