...
async function main() {
  ...
  console.log(JSON.stringify({ mode, model: result.model, latencyMs, timings: result.timings, spatialGraph: result.spatialGraph ? { rooms: result.spatialGraph.rooms.map(room => ({ id: room.id, bounds: room.bounds, confidence: room.confidence })), connections: result.spatialGraph.connections.length, observations: result.spatialGraph.observations.map(observation => ({ id: observation.id, area: observation.area, confidence: observation.confidence })) } : null, evidence: result.evidence.length, findings: result.findings.map(f => ({ title: f.title, areaIds: f.areaIds })), costs: result.evidence.filter(e => e.sourceType === 'calculation').map(e => e.data), corrections: result.corrections, tools: result.events.filter(e => e.phase === 'started').map(e => e.tool) }, null, 2));
}
